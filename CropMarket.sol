...
// ----------------------------------------------------------------------------
// --- GrailMarket - Marketplace Contract
...
contract Events {
    ...
    function emitGrailSellOrderCreated(address, uint256, uint256) external;
    function emitGrailSellOrderCancelled(address) external;
    function emitGrailSold(address, address, uint256, uint256) external;
    function emitGrailBuyOrderCreated(address, uint256, uint256) external;
    function emitGrailBuyOrderCancelled(address) external;
    function emitGrailBought(address, address, uint256, uint256) external;
...
contract MainMarket is Pausable, Upgradable, HumanOriented {
    ...
    GrailMarketplace InternalMarketplace;
    Events events;
    ...
    function buyEgg(
        ...
        if (_success) {
            _transferEth(msg.sender, _seller, msg.value, _price, _isGrail);
    ...
    function sellEgg(
        ...
        marketplaceController.sellEgg(msg.sender, _id, _maxPrice, _minPrice, _period, _isGrail);
    ...
    function sellDragon(
        ...
        marketplaceController.sellDragon(msg.sender, _id, _maxPrice, _minPrice, _period, _isGrail);
    ...
    function sellBreeding(
        ...
        marketplaceController.sellBreeding(msg.sender, _id, _maxPrice, _minPrice, _period, _isGrail);
    ...
    function fillGrailSellOrder(
        address _seller,
        uint256 _price,
        uint256 _amount
    ) external onlyHuman whenNotPaused payable {
        address(InternalMarketplace).transfer(msg.value);
        uint256 _priceForOne = InternalMarketplace.fillSellOrder(msg.sender, msg.value, _seller, _price, _amount);
        events.emitGrailSold(msg.sender, _seller, _amount, _priceForOne);
    }

    function createGrailSellOrder(
        uint256 _price,
        uint256 _amount
    ) external onlyHuman whenNotPaused {
        InternalMarketplace.createSellOrder(msg.sender, _price, _amount);
        events.emitGrailSellOrderCreated(msg.sender, _price, _amount);
    }

    function cancelGrailSellOrder() external onlyHuman whenNotPaused {
        InternalMarketplace.cancelSellOrder(msg.sender);
        events.emitGrailSellOrderCancelled(msg.sender);
    }

    // BUY

    function fillGrailBuyOrder(
        address _buyer,
        uint256 _price,
        uint256 _amount
    ) external onlyHuman whenNotPaused {
        uint256 _priceForOne = InternalMarketplace.fillBuyOrder(msg.sender, _buyer, _price, _amount);
        events.emitGrailBought(msg.sender, _buyer, _amount, _priceForOne);
    }

    function createGrailBuyOrder(
        uint256 _price,
        uint256 _amount
    ) external onlyHuman whenNotPaused payable {
        address(InternalMarketplace).transfer(msg.value);
        InternalMarketplace.createBuyOrder(msg.sender, msg.value, _price, _amount);
        events.emitGrailBuyOrderCreated(msg.sender, _price, _amount);
    }

    function cancelGrailBuyOrder() external onlyHuman whenNotPaused {
        InternalMarketplace.cancelBuyOrder(msg.sender);
        events.emitGrailBuyOrderCancelled(msg.sender);
    }
    ...
    function setInternalDependencies(address[] _newDependencies) public onlyOwner {
        ...
        Internal
        Marketplace = GrailMarketplace(_newDependencies[1]);
